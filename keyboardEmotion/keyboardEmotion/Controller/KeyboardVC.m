...
@interface KeyboardEmoticonCell: UICollectionViewCell
/** 表情模型 */
@property (nonatomic, strong) KeyboardEmoticonModel *emotion;


@property (nonatomic, strong) KeyboardButton *emoticonButton;


+ (NSString *)identifier;
@end

@implementation KeyboardEmoticonCell

- (void)setEmotion:(KeyboardEmoticonModel *)emotion {
    UIImage *image = [UIImage imageWithContentsOfFile:emotion.imagePath];
    [self.emoticonButton setImage:image forState:UIControlStateNormal];
    [self.emoticonButton setTitle:emotion.code forState:UIControlStateNormal];
}
...
- (void)layoutSubviews {
    ...
    self.emoticonButton.frame = CGRectInset(self.bounds, 4, 4);
}
...
#pragma mark - lazy
- (KeyboardButton *)emoticonButton {
    if (!_emoticonButton) {
        _emoticonButton = [KeyboardButton new];
        _emoticonButton.titleLabel.font = [UIFont systemFontOfSize:12];
        [_emoticonButton setTitleColor:[UIColor darkGrayColor] forState:UIControlStateNormal];
...
#pragma mark - 自定义布局
@interface KeyboardEmoticonLayout: UICollectionViewFlowLayout

@end

@implementation KeyboardEmoticonLayout
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        NSArray *images = @[@"Keyboard_first_emoticon", @"Keyboard_first_emoticon", @"Keyboard_first_emoticon", @"Keyboard_first_emoticon", @"Keyboard_first_emoticon", @"Keyboard_first_emoticon", @"Keyboard_first_emoticon", @"Keyboard_first_emoticon"];
        CGFloat itemX = 0;
        CGFloat itemY = 0;
        CGFloat itemW = 62;
        CGFloat itemH = 37;
        UIImage *image = [UIImage imageWithColor:[UIColor colorWithHexString:@"f2f2f6"]];
        ...
        UIImageView *gradientMask = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"Keyboard_gradient_mask"]];
        ...
        [sendButotn setTitleColor:[UIColor colorWithHexString:@"ffffff"] forState:UIControlStateNormal];
        [sendButotn setBackgroundImage:[UIImage imageWithColor:[UIColor colorWithHexString:@"4285f4" alpha:0.3]] forState:UIControlStateHighlighted];
        [sendButotn setBackgroundImage:[UIImage imageWithColor:[UIColor colorWithHexString:@"4285f4"]] forState:UIControlStateNormal];
...
- (void)setupInit {
    self.view.backgroundColor = [UIColor colorWithHexString:@"f2f2f6"];
...
- (UICollectionView *)collectionVeiw {
    if (!_collectionVeiw) {
        _collectionVeiw = [[UICollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:[[KeyboardEmoticonLayout alloc] init]];
        _collectionVeiw.backgroundColor = [UIColor clearColor];
        [_collectionVeiw registerClass:[KeyboardEmoticonCell class] forCellWithReuseIdentifier:[KeyboardEmoticonCell identifier]];
...
- (NSArray *)packages {
    if (!_packages) {
        _packages = [KeyboardEmoticonPackage loadPackages];
    }
    return _packages;
}
...
- (UIPageControl *)pageControl {
    if (!_pageControl) {
        ...
        KeyboardEmoticonPackage *package = self.packages.firstObject;
        _pageControl.numberOfPages = package.emoticons.count / 8;
        _pageControl.currentPage = 0;
        _pageControl.pageIndicatorTintColor = [UIColor colorWithHexString:@"e3e3e8"];
        _pageControl.currentPageIndicatorTintColor = [UIColor colorWithHexString:@"bababf"];
    }
    return _pageControl;
}
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    KeyboardEmoticonPackage *package = self.packages[section];
    return package.emoticons.count;
}


- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    KeyboardEmoticonCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:[KeyboardEmoticonCell identifier] forIndexPath:indexPath];
    KeyboardEmoticonPackage *package = self.packages[indexPath.section];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    KeyboardEmoticonPackage *package = self.packages[indexPath.section];
    KeyboardEmoticonModel *emotion = package.emoticons[indexPath.item];
    
    !self.emoticonCallBack ? : self.emoticonCallBack(emotion);
}
